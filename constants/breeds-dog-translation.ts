export const breedsDogTranslation = {
  Affenpinscher: 'Affenpinscher',
  'Afghan Hound': 'Galgo Afegão',
  'African Hunting Dog': 'Cão de Caça Africano',
  'Airedale Terrier': 'Airedale Terrier',
  'Akbash Dog': 'Cão de Akbash',
  Akita: 'Akita',
  'Alapaha Blue Blood Bulldog': 'Buldogue Alapaha Sangue Azul',
  'Alaskan Husky': 'Husky do Alasca',
  'Alaskan Malamute': 'Malamute do Alasca',
  'American Bulldog': 'Buldogue Americano',
  'American Bully': 'Bully Americano',
  'American Eskimo Dog': 'Cão Esquimó Americano',
  'American Eskimo Dog (Miniature)': 'Cão Esquimó Americano (Miniatura)',
  'American Foxhound': 'Foxhound Americano',
  'American Pit Bull Terrier': 'Pit Bull Terrier Americano',
  'American Staffordshire Terrier': 'Staffordshire Terrier Americano',
  'American Water Spaniel': "Spaniel d'Água Americano",
  'Anatolian Shepherd Dog': 'Cão Pastor da Anatólia',
  'Appenzeller Sennenhund': 'Cão Sennenhund de Appenzell',
  'Australian Cattle Dog': 'Cão Pastor Australiano',
  'Australian Kelpie': 'Kelpie Australiano',
  'Australian Shepherd': 'Pastor Australiano',
  'Australian Terrier': 'Terrier Australiano',
  Azawakh: 'Azawakh',
  Barbet: 'Barbet',
  Basenji: 'Basenji',
  'Basset Bleu de Gascogne': 'Basset Azul da Gasconha',
  'Basset Hound': 'Basset Hound',
  Beagle: 'Beagle',
  'Bearded Collie': 'Collie Barbeiro',
  Beauceron: 'Beauceron',
  'Bedlington Terrier': 'Terrier Bedlington',
  'Belgian Malinois': 'Malinois Belga',
  'Belgian Tervuren': 'Tervuren Belga',
  'Bernese Mountain Dog': 'Cão de Montanha de Bernese',
  'Bichon Frise': 'Bichon Frisé',
  'Black and Tan Coonhound': 'Coonhound Preto e Castanho',
  Bloodhound: 'Bloodhound',
  'Bluetick Coonhound': 'Coonhound Bluetick',
  Boerboel: 'Boerboel',
  'Border Collie': 'Border Collie',
  'Border Terrier': 'Terrier de Fronteira',
  'Boston Terrier': 'Boston Terrier',
  'Bouvier des Flandres': 'Boiadeiro da Flandre',
  Boxer: 'Boxer',
  'Boykin Spaniel': 'Spaniel Boykin',
  'Bracco Italiano': 'Braco Italiano',
  Briard: 'Briard',
  Brittany: 'Brittany',
  'Bull Terrier': 'Bull Terrier',
  'Bull Terrier (Miniature)': 'Bull Terrier (Miniatura)',
  Bullmastiff: 'Bullmastiff',
  'Cairn Terrier': 'Terrier Cairn',
  'Cane Corso': 'Cane Corso',
  'Cardigan Welsh Corgi': 'Corgi Galês Cardigan',
  'Catahoula Leopard Dog': 'Cão-leopardo Catahoula',
  'Caucasian Shepherd (Ovcharka)': 'Pastor Caucasiano (Ovcharka)',
  'Cavalier King Charles Spaniel': 'Spaniel Cavalier King Charles',
  'Chesapeake Bay Retriever': 'Retriever da Baía de Chesapeake',
  'Chinese Crested': 'Crestado Chinês',
  'Chinese Shar-Pei': 'Shar-Pei Chinês',
  Chinook: 'Chinook',
  'Chow Chow': 'Chow Chow',
  'Clumber Spaniel': 'Spaniel Clumber',
  'Cocker Spaniel': 'Cocker Spaniel',
  'Cocker Spaniel (American)': 'Cocker Spaniel Americano',
  'Coton de Tulear': 'Coton de Tuléar',
  Dalmatian: 'Dálmata',
  'Doberman Pinscher': 'Pinscher Doberman',
  'Dogo Argentino': 'Dogo Argentino',
  'Dutch Shepherd': 'Pastor Holandês',
  'English Setter': 'Setter Inglês',
  'English Shepherd': 'Pastor Inglês',
  'English Springer Spaniel': 'Springer Spaniel Inglês',
  'English Toy Spaniel': 'Spaniel Toy Inglês',
  'English Toy Terrier': 'Terrier Toy Inglês',
  Eurasier: 'Eurasier',
  'Field Spaniel': 'Spaniel Field',
  'Finnish Lapphund': 'Lapphund Finlandês',
  'Finnish Spitz': 'Spitz Finlandês',
  'French Bulldog': 'Bulldog Francês',
  'German Pinscher': 'Pinscher Alemão',
  'German Shepherd Dog': 'Pastor Alemão',
  'German Shorthaired Pointer': 'Pointer Alemão de Pêlo Curto',
  'Giant Schnauzer': 'Schnauzer Gigante',
  'Glen of Imaal Terrier': 'Terrier Glen of Imaal',
  'Golden Retriever': 'Golden Retriever',
  'Gordon Setter': 'Setter Gordon',
  'Great Dane': 'Dinamarquês',
  'Great Pyrenees': 'Pastor dos Pirenéus',
  Greyhound: 'Galgo',
  'Griffon Bruxellois': 'Griffon Bruxelense',
  Harrier: 'Harrier',
  Havanese: 'Havanese',
  'Irish Setter': 'Setter Irlandês',
  'Irish Terrier': 'Terrier Irlandês',
  'Irish Wolfhound': 'Lébrel Irlandês',
  'Italian Greyhound': 'Galgo Italiano',
  'Japanese Chin': 'Spaniel Japonês',
  'Japanese Spitz': 'Spitz Japonês',
  Keeshond: 'Keeshond',
  Komondor: 'Komondor',
  Kooikerhondje: 'Kooikerhondje',
  Kuvasz: 'Kuvasz',
  'Labrador Retriever': 'Labrador Retriever',
  'Lagotto Romagnolo': 'Lagotto Romagnolo',
  'Lancashire Heeler': 'Lancashire Heeler',
  Leonberger: 'Leonberger',
  'Lhasa Apso': 'Lhasa Apso',
  Maltese: 'Maltês',
  'Miniature American Shepherd': 'Pastor Americano em Miniatura',
  'Miniature Pinscher': 'Pinscher Miniatura',
  'Miniature Schnauzer': 'Schnauzer Miniatura',
  Newfoundland: 'Terra Nova',
  'Norfolk Terrier': 'Terrier de Norfolk',
  'Norwich Terrier': 'Terrier de Norwich',
  'Nova Scotia Duck Tolling Retriever': 'Retriever de Tolling do pato da Nova Escócia',
  'Old English Sheepdog': 'Pastor Inglês Antigo',
  'Olde English Bulldogge': 'Bulldog Inglês Antigo',
  Papillon: 'Cão Borboleta',
  Pekingese: 'Pequinês',
  'Pembroke Welsh Corgi': 'Corgi Galês Pembroke',
  'Perro de Presa Canario': 'Cão de Presa Canário',
  'Pharaoh Hound': 'Cão do Faraó',
  Plott: 'Plott Hound',
  Pomeranian: 'Pomerânia',
  'Poodle (Miniature)': 'Poodle Miniatura',
  'Poodle (Toy)': 'Poodle (Toy)',
  Pug: 'Pug',
  Puli: 'Puli',
  Pumi: 'Pumi',
  'Rat Terrier': 'Terrier Americano de Rato',
  'Redbone Coonhound': 'Coonhound Redbone',
  'Rhodesian Ridgeback': 'Ridgeback de Rodesia',
  Rottweiler: 'Rottweiler',
  'Russian Toy': 'Russo (Toy)',
  'Saint Bernard': 'São Bernardo',
  Saluki: 'Saluki',
  Samoyed: 'Samoieda',
  Schipperke: 'Schipperke',
  'Scottish Deerhound': 'Deerhound Escocês',
  'Scottish Terrier': 'Terrier Escocês',
  'Shetland Sheepdog': 'Pastor de Shetland',
  'Shiba Inu': 'Shiba Inu',
  'Shih Tzu': 'Shih Tzu',
  'Shiloh Shepherd': 'Pastor de Shiloh',
  'Siberian Husky': 'Husky Siberiano',
  'Silky Terrier': 'Terrier de Seda',
  'Smooth Fox Terrier': 'Fox Terrier de Pelo Liso',
  'Soft Coated Wheaten Terrier': 'Terrier Irlandês de Pelo Macio',
  'Spanish Water Dog': 'Cão de Água Espanhol',
  'Spinone Italiano': 'Spinone Italiano',
  'Staffordshire Bull Terrier': 'Staffordshire Bull Terrier',
  'Standard Schnauzer': 'Schnauzer Standard',
  'Swedish Vallhund': 'Vallhund Sueco',
  'Thai Ridgeback': 'Ridgeback Tailandês',
  'Tibetan Mastiff': 'Mastim Tibetano',
  'Tibetan Spaniel': 'Spaniel Tibetano',
  'Tibetan Terrier': 'Terrier Tibetano',
  'Toy Fox Terrier': 'Fox Terrier de Brinquedo',
  'Treeing Walker Coonhound': 'Coonhound de Caça',
  Vizsla: 'Vizsla',
  Weimaraner: 'Weimaraner',
  'Welsh Springer Spaniel': 'Springer Spaniel Galês',
  'West Highland White Terrier': 'Terrier Branco de Montanhas Ocidentais',
  Whippet: 'Whippet',
  'White Shepherd': 'Pastor Branco',
  'Wire Fox Terrier': 'Fox Terrier de Pelo Duro',
  'Wirehaired Pointing Griffon': 'Griffon de Aponte de Pelo Duro',
  'Wirehaired Vizsla': 'Vizsla de Pelo Duro',
  Xoloitzcuintli: 'Xoloitzcuintli',
  'Yorkshire Terrier': 'Yorkshire Terrier',
  Abyssinian: 'Abissínio',
  Aegean: 'Egeu',
  'American Bobtail': 'American Bobtail',
  'American Curl': 'American Curl',
  'American Shorthair': 'American Shorthair',
  'American Wirehair': 'American Wirehair',
  'Arabian Mau': 'Mau Árabe',
  'Australian Mist': 'Australian Mist',
  Balinese: 'Balinesa',
  Bambino: 'Bambino',
  Bengal: 'Bengal',
  Birman: 'Birmanês',
  Bombay: 'Bombaim',
  'British Longhair': 'British Longhair',
  'British Shorthair': 'British Shorthair',
  Burmese: 'Burmese',
  Burmilla: 'Burmilla',
  'California Spangled': 'California Spangled',
  'Chantilly-Tiffany': 'Chantilly-Tiffany',
  Chartreux: 'Chartreux',
  Chausie: 'Chausie',
  Cheetoh: 'Cheetoh',
  'Colorpoint Shorthair': 'Colorpoint Shorthair',
  'Cornish Rex': 'Cornish Rex',
  Cymric: 'Cymric',
  Cyprus: 'Chipre',
  'Devon Rex': 'Devon Rex',
  Donskoy: 'Donskoy',
  'Dragon Li': 'Dragão Li',
  'Egyptian Mau': 'Mau Egípcio',
  'European Burmese': 'Burmês Europeu',
  'Exotic Shorthair': 'Exotic Shorthair',
  'Havana Brown': 'Havana Brown',
  Himalayan: 'Himalaio',
  'Japanese Bobtail': 'Bobtail Japonês',
  Javanese: 'Javanês',
  'Khao Manee': 'Khao Manee',
  Korat: 'Korat',
  Kurilian: 'Kurilian',
  LaPerm: 'LaPerm',
  'Maine Coon': 'Maine Coon',
  Malayan: 'Malaia',
  Manx: 'Manx',
  Munchkin: 'Munchkin',
  Nebelung: 'Nebelung',
  'Norwegian Forest Cat': 'Gato da Floresta Norueguês',
  Ocicat: 'Ocicat',
  Oriental: 'Oriental',
  Persian: 'Persa',
  'Pixie-bob': 'Pixie-bob',
  Ragamuffin: 'Ragamuffin',
  Ragdoll: 'Ragdoll',
  'Russian Blue': 'Russo Azul',
  Savannah: 'Savannah',
  'Scottish Fold': 'Scottish Fold',
  'Selkirk Rex': 'Selkirk Rex',
  Siamese: 'Siamês',
  Siberian: 'Siberiano',
  Singapura: 'Singapura',
  Snowshoe: 'Snowshoe',
  Somali: 'Somali',
  Sphynx: 'Sphynx',
  Tonkinese: 'Tonquinês',
  Toyger: 'Toyger',
  'Turkish Angoraame': 'Angorá Turco',
  'Turkish Van': 'Van Turco',
  'York Chocolate': 'York Chocolate'
}
