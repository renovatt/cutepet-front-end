...
const { handleCreateDate, handleUpdateDate } = useDatePicker

watch(() => date.value, (newDate) => {
  if (newDate && typeof newDate === 'object') {
    handleCreateDate(newDate)
...
<template>
  <UPopover :popper="{ placement: 'bottom-start' }">
    <UButton class="transition-all" icon="i-heroicons-calendar-days-20-solid" :label="format(date, 'd MMM, yyy')" />

    <template #panel="{ close }">
      <VCalendarDatePicker v-model="date" v-bind="{ ...attrs, ...$attrs }" @close="close" />
    </template>
  </UPopover>
</template>
